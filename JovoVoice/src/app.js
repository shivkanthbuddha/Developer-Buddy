...
function checkFroEmails () {
    ...
    setTimeout(function () {
        getLastUpdatedFolder({ directory: attachmentsPath }, (lastUpdatedFolder = null) => {
            console.log("#### lastUpdatedFolder = " + lastUpdatedFolder);
            invokeSketch2Code(lastUpdatedFolder);
        });
        
    }, 6000);
    
       //setTimeout(checkFroEmails, 20000);
...
function isAnImage(data) {
    if (data.contentType.includes("image/gif") || data.contentType.includes("image/jpeg") || data.contentType.includes("image/png") || data.contentType.includes("image/jpg")) {
        return true
    }
    return false
}
...
function emptyFolder(dirPath, removeSelf) {
    ...
    async function sendEmail() {
        ...
        server.send({
            ...
            attachment:
                [
                    { data: "Hi , <br/> <br/> Please find attached generated HTML file and its  image from Voice2Code!  <br/> <br/> Thanks <br/> Voice2Code", alternative: true },
                    { path: destinationPath + "html.png", type: "image/png", name: "ScreenShot.png" },
                    { path: destinationPath +"result.html", type: "text/html", name: "index.html" },
...
function invokeSketch2Code(lastUpdatedFolder) {
    ...
    fs.readdir(directoryPath, function (err, files) {
        ...
        files.forEach(function (file) {
            ...
            if ( file.includes("jpg") || file.includes("png") || file.includes("jpeg") || file.includes("gif") ) {
                console.log("#### FileName = " + file);
                file = file.replace(".", "_");
                // empty the folder since the new html and image will be downloaded here
                emptyFolder(destinationPath, false)
                opn("http://localhost:34188/app/step2fromsample/" + lastUpdatedFolder + "-" + file);
               // opn("http://localhost:34188/ready-to-start/?folderFileName="   + lastUpdatedFolder + "-" + file );


                chokidar.watch(path.resolve(destinationPath + "result.html"), { ignored: /(^|[\/\\])\../ }).on('add', (event, path) => {
...
//getLastUpdatedFolder({ directory: attachmentsPath }, (lastUpdatedFolder = null) => { console.log("#### lastUpdatedFolder = " + lastUpdatedFolder); invokeSketch2Code(lastUpdatedFolder); });